# Adapted from:
# https://github.com/hansungkim98122/OpenEMMA-for-Waymo-E2E/blob/201f2c5b608aa1c5cbc5301f363c512ba0e0975e/waymo_e2e_dataset_sort.py
#
# Original author: hansungkim98122
# Modifications: Stores segment_id, num_frames, first_sequence_number, last_sequence_number, path in json
...
def list_tfrecord_files(input_dir: str) -> List[str]:
    ...
    for p in Path(input_dir).rglob("*"):
        ...
        if (
            ".tfrecord" in name
            or ".tfrecords" in name
            or ".tfrecord.gz" in name
            or ".tfrecord.gzip" in name
        ):
            files.append(str(p))
    files.sort()
    return files
...
def bucketize(input_files: List[str],
    ...
    if max_open_writers < num_buckets:
        raise ValueError(
            "max_open_writers must be >= num_buckets. "
            "TFRecordWriter does not append when reopening a file, so evicting "
            "bucket writers will silently truncate earlier bucket data. "
            f"Got num_buckets={num_buckets}, max_open_writers={max_open_writers}."
        )
    ...
    n = 0
    for rec in tqdm(ds, desc="bucketize records"):
        pb = bytes(rec.numpy())
...
def write_sorted_segments(bucket_paths: List[str],
                          output_dir: str,
                          output_compression: str) -> Tuple[Dict[str, int], List[Dict[str, object]]]:
    os.makedirs(output_dir, exist_ok=True)
    counts: Dict[str, int] = {}
    manifest_entries: List[Dict[str, object]] = []
    total = 0

    for bi, bpath in enumerate(tqdm(bucket_paths, desc="write buckets")):
        ...
        for seg, items in groups.items():
            ...
            first_sequence_number = int(items[0][0])
            last_sequence_number = int(items[-1][0])
            ...
            counts[seg] = counts.get(seg, 0) + num_frames
            total += num_frames
    ...
    manifest_entries.sort(key=lambda item: item["segment_id"])
    return counts, manifest_entries
...
def main():
    ...
    parser.add_argument("--max_open_writers", type=int, default=512,
                        help="Must be >= num_buckets because bucket TFRecord files cannot be safely reopened for append.")
    ...
    counts, manifest_entries = write_sorted_segments(
